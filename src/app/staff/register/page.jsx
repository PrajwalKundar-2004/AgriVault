    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 relative overflow-hidden px-4 sm:px-6 lg:px-8 font-sans text-slate-200">
      ...
      >
        <div className="bg-slate-800/30 py-5 px-5 sm:px-6 text-center relative overflow-hidden border-b border-slate-700/50">
          ...
          <motion.p variants={itemVariants} className="text-cyan-400/80 text-xs mt-1 relative z-10 font-medium">Create your staff account to continue</motion.p>
        </div>
        ...
        <div className="p-5 sm:p-6">
          <form onSubmit={handleRegister} className="space-y-3">
            ...
            <motion.div variants={itemVariants}>
              <label className="block text-xs uppercase tracking-wider font-bold text-slate-400 mb-2">Full Name</label>
              <input 
                type="text" 
                name="name"
                required 
            ...
            <motion.div variants={itemVariants} className="relative">
              ...
              <input 
                ...
                onChange={handleInputChange}
                className="w-full px-4 py-3.5 text-sm rounded-xl border border-slate-700/50 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all bg-slate-800/50 text-white placeholder-slate-500 shadow-inner pr-10"
                placeholder="••••••••"
              />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-[36px] text-slate-400 hover:text-blue-400 transition-colors">
                {showPassword ? (
            ...
            <motion.div variants={itemVariants} className="pt-2 text-center">
              ...
              <Link href="/staff/login" className="text-cyan-400 hover:text-cyan-300 text-sm font-bold transition-colors">
                Log In
              </Link>
            </motion.div>
